...
//		10004 박명수 7130
		...
		for(int i=0; i<temp.length; i++) {
			...
			if(check == false) {
				tempList[size][0] = info[0]; 
				tempList[size][1] = info[1]; 
				size += 1;
			}
		}
		...
		for(int i=0; i<size; i++) {
			...
			for(int j=0; j<temp.length; j++) {
				String[] info = temp[j].split("/");

				if(result[i].number == Integer.parseInt(info[0])) {
					result[i].price += Integer.parseInt(info[2]);
				}
			}
		}
...
//		메서드를 구현하시오.
		...
		for(int i=0; i<nodeList.length; i++) {
			if(maxPrice < nodeList[i].price) {
				maxPrice = nodeList[i].price;
				maxIndex = i;
			}
		}
		
		result = nodeList[maxIndex];
...
public class 메서드클래스배열4_문제02_리턴2_정답 {
