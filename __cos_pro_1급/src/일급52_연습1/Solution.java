...
주어진 단어가 순서대로 `["nice", "happy", "hello", "world", "hi"]` 인 경우 각 줄에 다음과 같이 적을 수 있습니다.('_'는 공백을 나타냅니다.)
...
한 줄에 적을 수 있는 글자 수 K와 적을 단어가 순서대로 담긴 배열 words가 매개변수로 주어질 때, 
...
class Solution {
 public int solution(int K, String[] words) {
     ...
     data += temp;
     answer = data.length() / (K + 1);
     if(data.length() % (K + 1) > 0) {
    	 answer += 1;
     }
     System.out.println(data);
     return answer;
