...
오늘 하루 동안의 지하철 도착 시각이 순서대로 들어있
는 배열 subwayTimes와 현재시간 currentTime이 매개변수로 주어질 때,
 지하철을 타기 위해 기다려야 하는 최소 대기 시간을 return 하도록 solution 메소드를 작성했습니다. 
 이때, 위 구조를 참고하여 중복되는 부분은 func_a라는 메소드로 작성했습니다. 
 코드가 올바르게 동작할 수 있도록 빈칸을 알맞게 채워 전체 코드를 완성해주세요.

---
#####매개변수 설명
오늘 하루 동안의 지하철 도착 시각이 순서대로 들어있는 배열 subwayTimes와 
현재시간 currentTime이 solution 메소드의 매개변수로 주어집니다. 
...
class Solution {
	...
	int getSecond(String time) {
		...
		String secondStr = time.substring(3);
		System.out.println(hourStr + " " + secondStr);
		int hour = Integer.parseInt(hourStr);
		int second = Integer.parseInt(secondStr);
		total = hour * 60 + second;
    ...
    public int solution(String[] subwayTimes, String currentTime) {
    		...
    		int curTime = getSecond(currentTime);
    		
    		for(int i = 0; i < subwayTimes.length; i++) {
  
    			int nexTime =  getSecond(subwayTimes[i]);
        		if(curTime <= nexTime) {
