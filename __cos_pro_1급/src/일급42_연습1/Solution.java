...
class Solution {
    public int solution(int[][] walls) {
        ...
        for(int i = 0; i < walls.length; i++) {
        	int total = 0;
        	int aWidth = walls[i][0];
        	int aHeight = walls[i][1];
        	for(int j = i + 1; j < walls.length; j++) {
        		int bWidth = walls[j][0];
        		int bHeight = walls[j][1];
        		int cWidth = bWidth - aWidth;
        		if(bHeight >= aHeight) {
        			total = aHeight * cWidth;
        		}else {
        			total = bHeight * cWidth;
        		}
        		System.out.println(aWidth + " " + aHeight + " " + bWidth + " " + bHeight);
